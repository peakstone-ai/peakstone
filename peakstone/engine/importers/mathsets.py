...
DEFAULT_SOURCES = [
    ...
    # Hendrycks MATH (hendrycks/competition_math) is intentionally NOT a default source: it was
    # DMCA'd off the Hugging Face Hub, i.e. it's exactly the copyright-encumbered content we exclude.
    # Competition-math coverage comes from our own native challenges/math/ instead. The `boxed` kind
    # + --source remain for anyone who imports a permissively-licensed boxed-answer set themselves.
]


def _fetch(src: Source) -> list[dict]:
    """Load rows via the `datasets` library (honors the local HF cache/token; the datasets-server
    REST API now 401s without auth)."""
...
# Per-dataset attribution, emitted only for sources actually imported (so we never claim a set we
# didn't ship). MIT-licensed sets only — copyright-encumbered ones must not be imported by default.
_ATTRIBUTION = {
    "openai/gsm8k": "**GSM8K** — `openai/gsm8k`, MIT. Cobbe et al., *Training Verifiers to Solve "
                    "Math Word Problems*, arXiv:2110.14168 (2021).",
}


def _source_doc(per_source, total) -> str:
    labels = ", ".join(src.label for src, _ in per_source) or "none"
    lines = [
        f"# mathsets ({labels})",
        "",
    ...
    lines += ["", "## Attribution & License", ""]
    for src, _ in per_source:
        lines.append("- " + _ATTRIBUTION.get(
            src.dataset,
            f"**{src.label}** — `{src.dataset}`. Verify the upstream license before publishing."))
    lines += [
        "",
        "The final answers stored in `expect` are extracted from each set's own solution field; "
        "the problem statements are reproduced verbatim in `spec.md`.",
        "",
    ]
    return "\n".join(lines)
